...
- **Rule 8 (Log Messages Are Documentation):** A log or assertion message is read at the worst possible moment. Say what failed, on which object, and what the reader should do about it; pass the object as the context argument (`Debug.Assert(_gridLayout != null, BoardLogMessages.GridLayoutConfigurationMissing, this)`), and keep the text as a `const` in the feature's message class. A class with no `UnityEngine.Object` of its own has two shapes, and the deciding question is whether it holds one worth opening. A **stateless rule** — a `*Validator` or `*Resolver` that only reads board state — returns a diagnostic and lets its MonoBehaviour caller write the message with `this` (see `AbilityDiagnostic`); that keeps the rule a pure function and lets a test assert on the flag instead of matching a log line. A **plain service that was injected a `UnityEngine.Object`** logs directly against that object (see `MatchInitializer`, which logs setup faults against the presenter the reader has to open). Routing those through the caller would strip the detail that makes them actionable — the authored index of the placement that failed cannot survive a bare result enum — and would force diagnostics into a domain type. The test on which shape applies is the console line: it must name an object the reader can click, and the object must be the one whose state is wrong.
