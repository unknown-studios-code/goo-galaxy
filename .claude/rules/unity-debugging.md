...
- **Editor Automation** → [unity-editor-automation.md](unity-editor-automation.md) (How to reach the running editor to read the console, inspect live objects, and evaluate code)
...
- **Rule 2 (Evidence Before Hypothesis):** Collect the exact error text and full stack trace, the reproduction steps, whether the failure is deterministic, and what changed just before it appeared. In the stack trace, read the first frame **inside project code** — the deepest engine frame is rarely the defect. **Fetch that evidence yourself rather than asking for it:** the editor is reachable from the shell, so `npm run unity:console` reads the console, `unity cmd get_performance_stats` reads frame timing, and `unity cmd get_serialized_fields` / `eval` read live object state. Mark the console first with `npm run unity:console:mark` — the buffer holds the whole session, and the PlayMode suite logs errors it asserts on deliberately, so an unmarked read is mostly other people's noise. Ask the user only for what the editor genuinely cannot answer: what they did, what they expected, and whether it reproduces.
