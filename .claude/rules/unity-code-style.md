...
This document defines the C# style, formatting, naming, and architectural conventions for the codebase. These rules ensure maintainability, readability, and performance across all game systems. Everything here is enforced or reinforced by `.editorconfig`, CSharpier (`printWidth 160`), and `dotnet format` — when in doubt, run `npm run format` and let the tooling settle the formatting.
