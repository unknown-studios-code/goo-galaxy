...
| Refresh `.csproj` / `.slnx` before `dotnet format`  | `recompile`; `SyncAll()` when it reports `up_to_date`                 | Trusting a stale csproj, or judging by mtime alone     |
