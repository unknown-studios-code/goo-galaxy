...
- **Rule 18a (Every Test Run Leaves A Ghost In The Console):** `com.unity.pipeline` registers a `TestResultCollector` with `CallbacksDelegator` on every run and never unregisters it on the success path — `RegisterCallbacks` has three call sites in `PipelineTestRunner.cs`, `UnregisterCallbacks` has one, reachable only for a cancelled run, and the success lambda nulls the field that guard tests before it can fire. `CallbacksDelegator.TryInvokeAllCallbacks` then delivers `RunFinished` to every stale collector, and `TestResultCollector.RunFinished` calls `SetResult` unguarded (line 88) where its own `Cancel` and `SetError` both check `IsCompleted` first. The result is an unattributed `InvalidOperationException` from `TaskCompletionSource.SetResult`, **once per prior run in the session**, growing by one each run and cleared only by restarting the Editor — measured here as 0 after a restart, then exactly 1, then exactly 2. It fails no test, it is not project code, and it is invisible to `LogAssert` because it is raised after the run ends. Two traps follow. The count tracking runs rather than tests makes a coincidental match with a fixture's test count look causal — chase the **full** stack, not the first line, because `npm run unity:console` prints only the first and that is what hides the package frames. And a green suite read straight afterwards will look like it errored: judge `npm run unity:console` against this and against the `LogAssert.Expect` errors the PlayMode suite raises on purpose (Rule 18), never on the raw error count.
