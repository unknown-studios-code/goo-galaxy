...
- **Rule 14 (Assembly Wiring):** Test assemblies keep `"defineConstraints": ["UNITY_INCLUDE_TESTS"]`, `"autoReferenced": false`, and `nunit.framework.dll` in `precompiledReferences`; EditMode also keeps `"includePlatforms": ["Editor"]`. Add the assembly under test to `references` and expose its internals with `[assembly: InternalsVisibleTo("GooGalaxy.Runtime.Tests.EditMode")]` in that assembly's `AssemblyInfo.cs` — never by widening a member to `public` for a test. State every `.asmdef` change in the handoff, since the file is edited but the recompile happens in the editor.
