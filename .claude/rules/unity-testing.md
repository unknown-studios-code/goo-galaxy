...
- **Class Organization** → [unity-class-organization.md](unity-class-organization.md) (Rule 9 states the fixture's member order: constants → fields → setup/teardown → tests → private helpers → nested fakes)
...
- **Rule 2 (Naming & Structure):** One `[TestFixture]` per type under test, in a file named `<TypeUnderTest>Tests.cs`, in a namespace mirroring the runtime folder. Name every test `MethodUnderTest_Scenario_ExpectedOutcome`. **A PlayMode fixture exercising an integration flow across several components is the one exception**: it is named for the flow rather than for a type — `MatchOvertimeTests`, `MatchDominationTests`, `GridInitializationOrderTests`, `OpeningBoardRenderTests` — and names its tests `Trigger_Scenario_ExpectedOutcome`, because no single member is under test. Reach for it only when the behavior genuinely spans components and no one type owns the outcome; a flow name is not a licence to avoid deciding what a test is about, and folding such a flow into the fixture of whichever type happens to host it produces a thousand-line file that nobody can navigate. Partition the body with literal `// GIVEN`, `// WHEN`, and `// THEN` comments, in that order, with the act step as a single statement. A combined marker — `// GIVEN / WHEN` or `// WHEN / THEN` — is permitted only where the two adjacent phases genuinely collapse into that one statement; never combine all three. Keep an empty `// GIVEN` when nothing needs arranging, since a missing marker reads as an oversight and an empty one reads as a decision. A `yield return null` immediately after the act, there only to let its effect land, stays inside `// WHEN`. A test that only compares an invariant, with neither arrange nor act, may carry `// THEN` alone.
...
- **Rule 13 (What Not to Test):** Do not test the engine, third-party packages, private helpers, or property pass-throughs. Do test boundaries, invalid input, and the exact edge that broke: every bug fix ships with a regression test named after the defect it prevents. A test **may** assert an allocation guarantee when that guarantee is documented in a `<remarks>` on the code under test, with `Assert.That(code, new AllocatesNothingConstraint())` from `GooGalaxy.Tests.Utils` — it counts managed allocations on the calling thread with a private `ProfilerRecorder` and names the count when it fails. Warm the path up first (a first call's JIT and type initialization allocate), measure N iterations in one delegate, and mark the test `[Category("Allocation")]` so the fast loop can exclude it. Two other primitives are banned, both measured on 6000.3.18f1: `GC.GetAllocatedBytesForCurrentThread()` returns 0 in this runtime, so a before/after delta on it passes whatever the code does; and Unity's `Is.Not.AllocatingGCMemory()` re-scopes the process-wide `GC.Alloc` recorder around every measurement and counts another thread's allocation made during that switch, failing allocation-free code at random on CI (GOOM-36).
- **Rule 14 (Assembly Wiring):** Test assemblies keep `"defineConstraints": ["UNITY_INCLUDE_TESTS"]`, `"autoReferenced": false`, and `nunit.framework.dll` in `precompiledReferences`; EditMode also keeps `"includePlatforms": ["Editor"]`. Add the assembly under test to `references` and expose its internals with `[assembly: InternalsVisibleTo("GooGalaxy.Tests.EditMode")]` in that assembly's `AssemblyInfo.cs` — never by widening a member to `public` for a test. Helpers shared by both suites live in `Assets/Scripts/Tests/Utils/` as `GooGalaxy.Tests.Utils`, with the same define constraint, `autoReferenced: false` and nunit reference but no platform restriction, since the PlayMode suite must build for players; the suites reference it, it references no suite, and a test of a helper lives in the suite folder that mirrors its location (`Tests/EditMode/Utils/`). State every `.asmdef` change in the handoff, since the file is edited but the recompile happens in the editor.

- **Rule 15 (Flaky and Disabled Tests):** An intermittent failure is a defect in the test or in the code, never noise. Fix it or delete it the day it appears — a suite people re-run until it turns green has stopped being a signal. Never comment a test out, and never leave `[Ignore]` without a tracker ID and the condition for re-enabling it (`[Ignore("GOOM-42: pending the deterministic tick")]`). Local runs and CI run the same set of tests: do not gate a test with `[Explicit]` or `[UnityPlatform]`. CI's `-assemblyNames` selection runs `[Explicit]` tests the local scripts skip, and a platform gate leaves the Windows editor as the only place a test runs, so either one splits what is verified. When the behaviour depends on the platform — a file lock, a device — fake the platform through an `internal` seam instead (see `JsonlFileSink`'s delete seam).
