...
- **Rule 13 (What Not to Test):** Do not test the engine, third-party packages, private helpers, or property pass-throughs. Do test boundaries, invalid input, and the exact edge that broke: every bug fix ships with a regression test named after the defect it prevents. An EditMode test **may** assert an allocation guarantee with `GC.GetAllocatedBytesForCurrentThread()` when that guarantee is documented in a `<remarks>` on the code under test — measure a delta across N iterations, never an absolute figure, because an absolute breaks between Mono and IL2CPP and under Test Runner overhead, and mark it `[Category("Allocation")]` so the fast loop can exclude it.
