...
tools: [read, search, edit, read/problems, vscodeTasks/problems]
---
...
4. Build fixtures in code with explicit seeds and injected fakes. Prefer constructor/method injection over `ServiceLocator` lookups in tests.
5. Name tests `MethodUnderTest_Scenario_ExpectedOutcome`. Follow the same style and member-ordering rules as production code (`.github/instructions/`).
6. Tear down every spawned `GameObject`, `ScriptableObject`, and network object in `[TearDown]`/`[UnityTearDown]`.
7. Verify with the errors tool only. Then hand off — tell the user to run the suite.
