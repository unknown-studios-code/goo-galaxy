...
`Editor.Shared` is the only assembly the other editor assemblies depend on. C# conventions in `.github/instructions/` apply, including member ordering and documentation scope. Domain-reload behavior is governed by `unity-project-configuration.instructions.md` — static state in editor tools must survive or reset deliberately.
...
7. Verify with the errors tool, then run `npm run format`.
