...
4. **Fetch Notion data** — use the `track-task` skill / Notion MCP search to look up the task and parent story by their IDs. Pull the task name, priority, type, acceptance criteria (Definition of Done), and page URL.
5. **Generate the title** — format as `type(scope): subject` using the rules in the Title Format section below.
6. **Read the template** — read `templates/pr-template.md` and use it as the body structure.
...
11. **Update Notion** — use Notion MCP to update the task page with the branch name (`Branch` property) and PR URL (`Pull Request` property).
12. **Report** — return the PR number, URL, applied labels, and whether the Notion sync succeeded.
...
Use `templates/pr-template.md` as the structural source. Fill in every section with real data, never leaving placeholders.
...
Do not fabricate old folder structures or architecture buckets that do not exist in this repository.
...
- The head branch is pushed and is not `main`

After opening a PR, verify:

- The PR exists and returns a number and URL
- Labels were applied successfully
- The Notion task carries the branch name and PR URL
- Report all of the above to the user
