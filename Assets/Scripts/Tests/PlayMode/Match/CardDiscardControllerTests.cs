...
namespace GooGalaxy.Tests.PlayMode.Match
{
    ...
    public class CardDiscardControllerTests
    {
        ...
        public void TryDiscardCard_MatchNotInStandardPhase_ReturnsMatchNotInPlayAndLeavesHandCycleAndEnergyUnchanged(MatchPhase phase)
        {
            // GIVEN
            DeckPresenter deckPresenter = BuildDeckPresenter(HandSize);
            var ledger = new FakeDiscardLedger();
            MatchController matchController = BuildMatchController(phase);
            CardDiscardController controller = BuildDiscardController(deckPresenter, ledger, BuildBareDeployController(), matchController);
            ...
            Assert.That((result, cardAfter, nextAfter, ledger.PayCalls.Count), Is.EqualTo((CardDiscardResult.MatchNotInPlay, cardBefore, nextBefore, 0)));
        }
        ...
        public void TryDiscardCard_MissingDeployController_ReturnsDeckUnavailableAndLeavesHandCycleAndEnergyUnchanged()
        {
            ...
            DeckPresenter deckPresenter = BuildDeckPresenter(HandSize);
            var ledger = new FakeDiscardLedger();
            LogAssert.Expect(LogType.Assert, MatchLogMessages.DiscardDeployControllerMissing);
        ...
        public void TryDiscardCard_MissingDeckPresenter_ReturnsDeckUnavailableAndChargesNothing()
        {
            // GIVEN
            var ledger = new FakeDiscardLedger();
            LogAssert.Expect(LogType.Assert, MatchLogMessages.DiscardCardCycleMissing);
        ...
        public void TryDiscardCard_MissingDiscardLedger_ReturnsDeckUnavailableAndLeavesHandCycleUnchanged()
        {
            // GIVEN
            DeckPresenter deckPresenter = BuildDeckPresenter(HandSize);
            LogAssert.Expect(LogType.Assert, MatchLogMessages.DiscardLedgerMissing);
        ...
        {
            return BuildDiscardController(deckPresenter, ledger, deployController, _matchController);
        }
        ...
        // Never activated: SetPhaseForTests mutates MatchState directly and TryPlayCard/TryDiscardCard only ever
        // read MatchController.Phase, so nothing here needs Awake, OnEnable, or the countdown to have run.
