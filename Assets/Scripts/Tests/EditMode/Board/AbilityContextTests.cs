...
namespace GooGalaxy.Tests.EditMode.Board
{
    ...
    public class AbilityContextTests
    {
        ...
        {
            // GIVEN / WHEN
            AbilityContext context = AbilityContext.ForLanding(ActingPlayerId, ActingUnitId, _landingHex, false, default, default);
        ...
        {
            // GIVEN / WHEN — the shape AbilityController builds when a Jump's target hex turned out empty.
            AbilityContext context = AbilityContext.ForLanding(ActingPlayerId, AbilityContext.NoActingUnit, _landingHex, false, default, default);
        ...
        {
            // GIVEN / WHEN — a troop derives its impact area from the landing hex, it is never handed one.
            AbilityContext context = AbilityContext.ForLanding(ActingPlayerId, ActingUnitId, _landingHex, false, default, default);
        ...
        {
            // GIVEN / WHEN — a Protocol puts no unit on the board, regardless of the targets it is given.
            AbilityContext context = AbilityContext.ForSpell(ActingPlayerId, new List<HexCoordinates> { _landingHex });
        ...
        {
            // GIVEN / WHEN — regression: HasExplicitTargets is stored by the factory, never inferred from list
            ...
            AbilityContext context = AbilityContext.ForSpell(ActingPlayerId, new List<HexCoordinates>());
        ...
        {
            // GIVEN / WHEN — regression: same as the empty-list case, a null target list must not make this read
            // as a troop landing. See ForSpell_WithEmptyTargetHexes_HasExplicitTargetsIsTrue for the full rationale.
            AbilityContext context = AbilityContext.ForSpell(ActingPlayerId, null);
        ...
        {
            // GIVEN / WHEN — the centre every impact's radius is measured against is targets[0], not a derived midpoint.
            AbilityContext context = AbilityContext.ForSpell(ActingPlayerId, new List<HexCoordinates> { _secondTargetHex, _landingHex });
        ...
        {
            // GIVEN / WHEN — "no unit here" and "no unit acting" must read as the same sentinel.
